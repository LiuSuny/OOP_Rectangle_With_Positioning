...
/*
К классу Rectangle дописать следующий функционал:

Отрисовать рамку 40 на 40.
Каждая сторона рамки своего цвета. Цвета генерируем случайным образом.
Прямоугольник перемещается внутри этой рамки.
Как только прямоугольник наехал на любую из сторон рамки - прямоугольник принял цвет
этой стороны рамки, сторона рамки сменила цвет на случайный.
*/
//класс Frame
//высота, ширина, 4 цвета, методы для установки значений в каждое поле, метод для возвращения значений, отрисовка рамки
class myFrame
{
	int height = 40;
	int width = 40;
...
public:
	...
	void SetColorRight(Color color)
	{
		colorright = color;
	}
...
void main()
{
	...
	{
		frameObj.printFrame();
		//определяем с какой стороной произошло столкновение
		code = rect.moveRect(frameObj.GetWidth(), frameObj.GetHeight());
		switch (code)
		{
		case Up:
		...
		case Right:
			...
			frameObj.SetColorRight(Color(1 + rand() % 15));
			break;
		case Down:
